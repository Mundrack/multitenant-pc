...
export function useOrganizations(organizationId?: string) {
  ...
  const loadSubOrganizations = useCallback(async (orgId: string) => {
    try {
      ...
      console.log('Sub-organizaciones cargadas:', data);
  ...
  const buildOrganizationTree = useCallback((
    ...
    const rootNode: OrganizationTree = {
      id: org.id,
      name: org.name,
      type: 'organization',
      level: 0,
    ...
    const buildChildren = (parentId: string, parentType: 'organization' | 'sub_organization'): OrganizationTree[] => {
      const children = subOrgs
        .filter(sub => {
          // Para nivel 1: buscar los que apuntan a la organización principal Y no tienen parent_sub_organization_id
          if (parentType === 'organization') {
            return sub.level === 1 && !sub.parent_sub_organization_id;
          }
          // Para nivel 2+: buscar los que tienen parent_sub_organization_id apuntando al padre
          return (sub as any).parent_sub_organization_id === parentId;
        })
        .map(sub => {
          ...
          node.children = buildChildren(sub.id, 'sub_organization');
    ...
    rootNode.children = buildChildren(org.id, 'organization');
    tree.push(rootNode);
  ...
  const createSubOrganization = async (input: {
    ...
    description?: string;
    manager_id?: string;
    parent_sub_organization_id?: string;
  }) => {
